                ...
                <div class="card-body">
                    ...
                    <div class="row mb-3">
                        <div class="col-12 text-end">
                            <a href="{{ route('dashboard.page-category.index') }}"
                                class="btn btn-secondary btn-sm me-1">{{ __('Back') }}</a>
                            <form action="{{ route("dashboard.page-category.restore") }}" method='POST' style='display:inline-block;' class="restore-form">
                                <input type="hidden" class="form-control" name="id" value="">
                                <button type="submit" id="btn-restore"
                                    class="btn btn-sm btn-primary me-1 restore" disabled>{{ __('Restore') }}</button>
                            </form>
                            <form action="{{ route("dashboard.page-category.delete") }}" method='POST' style='display:inline-block;' class="delete-form">
                                <input type="hidden" class="form-control" name="id" value="">
...
@push('js')
    <script>
        var _URL = "{{ route('trash.pages.category') }}"
        var _COLUMNS = [{
